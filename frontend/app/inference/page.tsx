 ...
 *
 * Reads the same evidence as Risk & distortions (shared evidence state via
 * localStorage backed by the same demo defaults). Runs the decomposed
 * inference endpoint, which returns the headline DO risk, the full posterior
 * distribution, per-family contributions to Node 20, and the list of nodes
 * saturated at the [0.05, 0.95] boundaries.
 *
 * No editing. Read-only. Toggle the collider-discount reading. Sort the
 * distribution table. Tap a saturated node to jump to Risk & distortions.
 */

// ── Evidence read — same default as Risk & distortions's DEMO_EVIDENCE ──────
// Phase A.5 unifies these under a real case state.
    ...
    <>
      ...
      <div style={{ padding: '24px 36px 64px', maxWidth: 1080 }}>
        ...
        <div className="mb-6">
          ...
          >
            The read-only analysis view. Renders the Bayesian network's full
            ...
            <Link href="/risk" className="underline" style={{ color: PV.distortion }}>
              Risk &amp; distortions
            </Link>.
          </p>
        </div>
...
function InferenceBody({
  ...
  return (
    <>
      {/* Headline */}
      <div
        className="grid items-start mb-7 pb-6 border-b border-border"
        style={{ gridTemplateColumns: '1.3fr 1fr', gap: 36 }}
      >
        <div>
          ...
          <div
            ...
            style={{ fontSize: 72, lineHeight: 1, color: band.color, letterSpacing: '-0.02em' }}
          >
          ...
          <div
            ...
            style={{ fontSize: 18, color: band.color }}
          >
            {band.label}
          </div>
          ...
          <div
            className="mt-5 pt-4 border-t flex items-start gap-3"
            style={{ borderColor: PV.border, maxWidth: 440 }}
          >
            <input
              type="checkbox"
              id="collider-toggle"
              ...
              style={{ accentColor: PV.distortion, width: 14, height: 14, marginTop: 3 }}
            />
            <label
              htmlFor="collider-toggle"
              className="cursor-pointer select-none"
              style={{ fontSize: 13, color: PV.ink2, lineHeight: 1.45 }}
            >
          ...
          {data.do_risk_collider_discounted != null && (
            <div className="mt-4 flex items-baseline gap-3">
              <span className="label-caps" style={{ color: PV.distortion }}>
                Collider-discounted reading
              </span>
              <span
                className="font-serif tabular-nums"
                style={{ fontSize: 22, color: PV.distortion }}
              >
        ...
        <div className="bg-paper2 rounded-xl border p-5" style={{ borderColor: PV.border }}>
          ...
          <Provenance
            label="Evidence"
            value={`${data.completeness.observed} of ${data.completeness.total_evidence_nodes} observed`}
          />
          <Provenance
            label="Shifts"
            value={
              data.shifts_applied && Object.keys(data.shifts_applied).length > 0
                ? `${Object.keys(data.shifts_applied).length} applied`
                : 'none'
            }
          />
      ...
      {/* Saturated nodes */}
      {data.saturated_nodes.length > 0 && (
        <SaturatedNodes ids={data.saturated_nodes} />
      )}
  ...
  return (
    <section className="mb-9">
      <div className="mb-3">
        ...
        <ItalicCaption>
          Each family's signed contribution to the DO risk under Ch.5 §5.1.20.
      ...
      <div className="flex flex-col gap-2">
        {data.families.map((fam) => {
          ...
          return (
            <div
              key={fam.family}
              className="bg-paper rounded-lg border"
              style={{
            ...
            >
              <div className="flex items-baseline gap-3 mb-2">
                <div
                  style={{
                    width: 4, height: 14, borderRadius: 2, alignSelf: 'center',
                ...
                <span
                  className="font-serif tabular-nums font-medium"
                  style={{ fontSize: 16, color: fam.color, minWidth: 70, textAlign: 'right' }}
                >
                ...
                {fam.nodes
                  ...
                  .map((n) => (
                    <span
                      ...
                      style={{ fontSize: 11.5, color: PV.ink3 }}
                    >
...
function SaturatedNodes({ ids }: { ids: string[] }) {
  return (
    <section className="mb-9">
      ...
      <ItalicCaption>
        These nodes are pinned at the engine's saturation bounds — either by
        hard evidence or by slider shifts pushing them to the limit. Saturation
        means the model can't distinguish gradations beyond this point. Consider
  ...
  const rows = Object.entries(posteriors)
    ...
    .map(([nid, p]) => {
      ...
      return {
        ...
        meta,
        posterior: p,
        shift:    shiftsApplied[nid] ?? 0,
        observed: nid in evidence,
      };
    })
    .filter((r) => r.meta);
    ...
    <section>
      <div className="flex items-baseline justify-between mb-3">
        <div>
          ...
          <ItalicCaption>
            Marginal posteriors for all 19 nodes returned by VE, plus the
            DO risk computed post-VE.
          </ItalicCaption>
        </div>
          ...
          {(['id', 'posterior', 'type'] as const).map((opt) => (
            <button
              ...
              style={{
                fontSize: 10,
                padding: '4px 8px',
                background: sortBy === opt ? PV.ink : 'transparent',
                color:      sortBy === opt ? '#fff' : PV.ink3,
                borderColor: sortBy === opt ? PV.ink : PV.border,
              }}
            >
              {opt}
            </button>
      ...
      <div
        className="rounded-xl border overflow-hidden bg-paper"
        style={{ borderColor: PV.border }}
      >
        {/* Header row */}
        <div
          ...
          style={{
            gridTemplateColumns: '8px 56px 1fr 80px 120px 80px',
            gap: 14,
            padding: '8px 14px',
            borderColor: PV.border,
          }}
        ...
        {rows.map((r) => {
          ...
          return (
            ...
            >
              <div
                style={{
                  width: 3, height: 22, borderRadius: 1.5, alignSelf: 'center',
                  background: color,
                }}
              />
              ...
              <div className="flex flex-col leading-tight min-w-0">
                ...
                <span
                  className="font-serif italic text-ink4 truncate"
                  style={{ fontSize: 11 }}
                >
                  {r.meta!.type}
                </span>
              ...
              <span
                ...
                style={{ fontSize: 11, color: r.observed ? PV.distortion : PV.ink4 }}
              >
                {r.observed ? 'yes' : '—'}
              </span>
              <span
                ...
                style={{
                  fontSize: 11,
                  color: Math.abs(r.shift) > 0.001
                    ? (r.shift > 0 ? PV.risk : PV.mitigation)
                    : PV.ink4,
                }}
              >
                {Math.abs(r.shift) > 0.001
                  ? `${r.shift > 0 ? '+' : ''}${(r.shift * 100).toFixed(1)}pp`
                  : '—'}
              </span>
              <span
...
function bandFor(p: number): { label: string; color: string } {
  if (p >= 0.70) return { label: 'High',     color: PV.risk        };
  if (p >= 0.55) return { label: 'Elevated', color: PV.constraint  };
  if (p >= 0.40) return { label: 'Moderate', color: '#BA9117'      };
